...
const nextConfig = {
  ...
  async headers() {
    ...
    const csp = [
      ...
      // CDN). Images only, clearly labelled CLAIMED, never on a trust-bearing
      // path. No other remote image host is allowed. Everything else stays
      // self/data/blob.
      "img-src 'self' data: blob: https://api.8004scan.io https://blob.8004scan.app",
      "font-src 'self' data:",
